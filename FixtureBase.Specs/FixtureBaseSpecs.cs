...

{
    public class FixtureBaseSpecs : FixtureBaseWithDbAndHttpFor<AUseCase>
    {
        ...
        [Fact]
        public void HttpClientShouldBeFakeHttpClient() { HttpClient.ShouldBeIn(Instances).ShouldBeOfType<FakeHttpClient>(); }
        ...
        public void UUTShouldNotBeNull()
        {
            try
            {
                ...
                Console.WriteLine("----------------");
                foreach (var kv in Activator.LastActivationTree) Console.WriteLine(kv.ToString(null));
                Console.WriteLine("----------------");
