...
namespace FixtureBase.Specs
{
    ...
    public class AUseCase
    {
        ...
        public AUseCase(IServiceRequiringDBandHttp service) { this.service = service; }

        public async Task<bool> PutHttp(string name) { return await service.PutHttp(name); }

        public async Task<string> GetHttp() { return await service.GetHttpAsync(); }

        public List<int> FromDb() { return service.GetFromDb(); }

        public List<Datum> FromDbIdAndNames() { return service.GetFromDbIdAndNames(); }

        public List<string> FromDbStrings() { return service.GetFromDbStrings(); }

        internal bool InsertDb(Datum newDatum) { return service.InsertDb(newDatum); }
    }
    ...
    class AServiceRequiringDBandHttp : IServiceRequiringDBandHttp
    {
        ...
        public bool InsertDb(Datum newDatum)
        {
            return db.Execute("Insert into Data (Id,Name) Values (@Id,@Name)", newDatum) > 0;
        }
    }
}
